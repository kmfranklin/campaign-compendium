...
<div class="max-w-5xl mx-auto bg-surface border border-border shadow rounded-lg p-6">
    ...
    <h1 class="text-2xl font-bold text-text mb-6">Edit Quest</h1>
    ...
    <form action="{{ route('campaigns.quests.update', [$campaign, $quest]) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="4"
                      class="mt-1 block w-full rounded-md border border-border bg-surface text-text shadow-sm
                             focus:border-accent focus:ring-accent sm:text-sm">{{ old('description', $quest->description) }}</textarea>
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea name="notes" id="notes" rows="3"
                      class="mt-1 block w-full rounded-md border border-border bg-surface text-text shadow-sm
                             focus:border-accent focus:ring-accent sm:text-sm">{{ old('notes', $quest->notes) }}</textarea>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="status" id="status"
                ...
                @foreach($statuses as $status)
                    <option value="{{ $status->value }}" @selected(old('status', $quest->status->value) === $status->value)>
                        {{ $status->label() }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="pt-4 border-t border-border flex justify-between">
            <a href="{{ route('campaigns.quests.show', [$campaign, $quest]) }}"
               class="px-4 py-2 bg-bg text-text rounded hover:bg-hover border border-border">
                Cancel
            </a>
